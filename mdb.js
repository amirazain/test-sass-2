...
//Initialization
Waves.attach('.btn, .btn-floating , .btn-flat', ['waves-light']);
Waves.attach('.waves-light', ['waves-light']);
Waves.attach('.navbar-nav a:not(.navbar-brand), .nav-icons li a, .navbar form, .nav-item', ['waves-light']);
	...
	$(document).ready(function () {
		...
		Materialize.updateTextFields = function () {
			...
			$(input_selector).each(function (index, element) {
				if ($(element).val().length > 0 || $(this).attr('placeholder') !== undefined || $(this).attr('v-model') !== undefined || $(element)[0].validity.badInput === true) {
		...
		$(document).on('reset', function (e) {
			...
			if (formReset.is('form')) {
				...
				formReset.find(input_selector).each(function () {
					if ($(this).attr('value') === '' || $(this).attr('v-model') === '') {
						$(this).siblings('label, i').removeClass('active');
					}
				});
		...
		init: function (options) {
			...
			return this.each(function () {
				...
				function captionTransition(caption, duration) {
					if (caption.hasClass("text-center")) {
						caption.velocity({
							opacity: 0,
							translateY: -100
						}, {
					...
					} else if (caption.hasClass("text-right")) {
						caption.velocity({
							opacity: 0,
							translateX: 100
						}, {
					...
					} else if (caption.hasClass("text-left")) {
						caption.velocity({
							opacity: 0,
							translateX: -100
						}, {
		...
		init: function (options) {
			...
			$(this).each(function () {
				...
				if (options.edge == 'left') {
					...
					menu.addClass('side-right') // Change text-alignment to right
...
// 					});
// 					$tabs_wrapper = $tabs_content.wrapAll('<div class="tab-content carousel"></div>');
